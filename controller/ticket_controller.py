...
--------------------
Handles all ticket-related routes for both the REST API (/api/tickets/...)
and the web UI (/tickets/...) and the dashboard (/dashboard).

Roles understood by this controller:
  - USER          -> can create and view their own tickets
  - SUPPORT_AGENT -> can view, update status/priority on tickets assigned to them
  - ADMIN         -> can do everything, including assigning tickets
...
def api_tickets():
    ...
    if request.method == "GET":
        ...
        tickets = ticket_service.get_all_ticket(
            status_filter, priority_filter, assigned_to_filter
        )
    ...
    ticket = ticket_service.add_ticket(
        title, description, category_id, current_user_id, priority
    )
    return jsonify(
        {"message": "Ticket Created Successfully", "ticket": ticket.to_dict()}
    ), 201


@ticket_bp.route("/api/tickets/<int:t_id>", methods=["GET"])
def api_view_single_ticket(t_id):
    """GET /api/tickets/<id> -> return a single ticket by its ID."""
...
def api_assign_ticket(t_id):
    ...
    Admin-only: assign a ticket to a support agent.
    Body: { "assigned_to": <agent_user_id> }
    """
    claims = get_jwt()
...
def api_modify_status(t_id):
    ...
    ADMIN and SUPPORT_AGENT only: change the status of a ticket.
    ...
    if role not in ["ADMIN", "SUPPORT_AGENT"]:
        return jsonify(
            {"message": "Only Admins or Support Agents can modify ticket status"}
        ), 403
...
def api_modify_priority(t_id):
    ...
    ADMIN and SUPPORT_AGENT only: escalate or de-escalate a ticket's priority.
    ...
    if role not in ["ADMIN", "SUPPORT_AGENT"]:
        return jsonify(
            {"message": "Only Admins or Support Agents can escalate tickets"}
        ), 403
    ...
    if ticket:
        return jsonify(
            {"message": "Priority Modified", "ticket": ticket.to_dict()}
        ), 200
    return jsonify({"message": "Invalid Ticket ID"}), 404
...
def api_upload_file(t_id):
    ...
    Upload a file attachment (png, jpg, jpeg, pdf) to a ticket.
    ...
    if file and allowed_file(file.filename):
        ...
        attachment = ticket_attachment_service.add_ticket_attachment(
            file_name=filename,
            file_path=save_path,
            ticket_id=t_id,
        ...
        return jsonify(
            {"message": "File uploaded successfully", "attachment": attachment.to_dict()}
        ), 201
...
def api_view_history(t_id):
    """GET /api/tickets/<id>/history -> return the status-change history of a ticket."""
...
def dashboard_page():
    """
    Dashboard page — what the user sees depends on their role:
      ADMIN         -> all tickets + aggregate statistics
      SUPPORT_AGENT -> tickets assigned to them (or unassigned)
    ...
    if role == "ADMIN":
        ...
        tickets = [
            ...
            if str(t.assigned_to) == str(user_id) or t.assigned_to is None
        ]
    else:
...
def create_ticket_page():
    """
    GET  /tickets/new -> render the create-ticket form
    POST /tickets/new -> process form submission and redirect to dashboard
    """
    if request.method == "GET":
...
def add_comment_page(t_id):
    """POST /tickets/<id>/comments -> add a comment and redirect back to detail."""
...
def update_ticket_web(t_id):
    ...
    ADMIN/SUPPORT_AGENT only: change status and/or priority via the web form.
    Regular users are silently redirected back to the ticket detail page.
    """
    claims = get_jwt()
