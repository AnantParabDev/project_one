...
test_tickets.py
---------------
Tests for ticket creation and retrieval (REST API endpoints).
...
def _auth_headers(client, email, role_id=1):
    """Register + login a user and return Authorization header dict."""
...
def test_create_ticket_no_auth(client):
    """Creating a ticket without a JWT must return 401."""
...
def test_get_tickets_no_auth(client):
    """Getting tickets without a JWT must return 401."""
...
def test_create_ticket(client):
    """An authenticated user can create a ticket; response contains the ticket."""
...
def test_create_ticket_response_fields(client):
    """Created ticket dict must include all expected fields."""
    headers = _auth_headers(client, "fieldticket@test.com")
    resp = client.post("/api/tickets", json={
        "title":       "Field Check Ticket",
    ...
    for field in ["ticket_id", "title", "description", "status", "priority",
                  "created_by", "created_at"]:
        assert field in ticket, f"Missing field: {field}"


def test_create_ticket_default_status_is_open(client):
    """Newly created tickets must have 'open' as their initial status."""
...
def test_create_ticket_high_priority_due_date_set(client):
    """High-priority tickets must have a due_date set (1-day SLA)."""
...
def test_get_all_tickets(client):
    """GET /api/tickets must return a JSON list."""
...
def test_get_ticket_by_valid_id(client):
    """GET /api/tickets/<id> with a valid ID must return the ticket."""
...
def test_get_ticket_invalid_id(client):
    """GET /api/tickets/<id> with a non-existent ID must return 404."""
...
def test_filter_tickets_by_status(client):
    """Tickets returned when filtering by status=open should all be open."""
